"""SQLAlchemy destination implementation."""
...
class SqlDestination(DatabaseDestination):
    """Base destination implementation for SQL databases via SQLAlchemy.
    ...
    dialect subclass (:class:`PostgresDestination`, :class:`MySQLDestination`,
    :class:`SqliteDestination`) which accepts explicit connection parameters and
    implements ``to_spec``.

    The destination is fully stateless with respect to table identity — the table
    name and schema are passed through from the asset context on every call, so a
