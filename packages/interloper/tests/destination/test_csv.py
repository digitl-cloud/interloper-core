...
class TestCsvDestination:
    ...
    def test_initialization(self, tmp_path):
        csv_dest = il.CsvDestination(base_path=str(tmp_path))
        assert csv_dest.base_path == str(tmp_path)
    ...
    def test_write_read_non_partitioned(self, tmp_path):
        ...
        csv_dest.write(context, data)
        result = csv_dest.read(context)
        assert result == data
    ...
    def test_write_read_partition_window(self, tmp_path):
        ...
        csv_dest.write(il.DestinationContext(asset=my_asset(), partition_or_window=window), data)

        # Reading a window returns a list per partition
        result = csv_dest.read(il.DestinationContext(asset=my_asset(), partition_or_window=window))
        assert len(result) == 3
    ...
    def test_partition_row_counts(self, tmp_path):
        ...
        csv_dest.write(il.DestinationContext(asset=asset_instance, partition_or_window=p1), [{"a": "1"}])
        csv_dest.write(
            il.DestinationContext(asset=asset_instance, partition_or_window=p2),
            [{"a": "1"}, {"a": "2"}, {"a": "3"}],
        )

        counts = csv_dest.partition_row_counts(il.DestinationContext(asset=asset_instance))
    ...
    def test_partition_row_counts_empty(self, tmp_path):
        ...
        counts = csv_dest.partition_row_counts(il.DestinationContext(asset=my_asset()))
        assert counts == {}

    def test_to_spec(self, tmp_path):
        csv_dest = il.CsvDestination(base_path=str(tmp_path))
        spec = csv_dest.to_spec()
        assert spec.config == {"base_path": str(tmp_path)}
        assert spec.init == {"key": "csv"}
        reconstructed = spec.reconstruct()
        assert isinstance(reconstructed, il.CsvDestination)
