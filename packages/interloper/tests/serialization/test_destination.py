"""Test ComponentInstanceSpec for Destination serialization."""
...
class TestDestinationSpec:
    """Test ComponentInstanceSpec serialization and reconstruction for Destination."""

    def test_destspec_roundtrip(self, tmp_path: Path):
        """Test ComponentInstanceSpec roundtrip (to_spec + reconstruct)."""
        # Create a FileDestination instance
        file_dest = il.FileDestination(base_path=str(tmp_path))

        # Convert to spec
        spec = file_dest.to_spec()
        assert isinstance(spec, ComponentInstanceSpec)
        assert spec.path == "interloper.destination.file.FileDestination"
        assert spec.config == {"base_path": str(tmp_path)}
        assert spec.init == {"key": "file"}

        # Reconstruct
        reconstructed = spec.reconstruct()
        assert isinstance(reconstructed, il.FileDestination)
        assert reconstructed.base_path == str(tmp_path)

    def test_destspec_validation(self):
        """Test ComponentInstanceSpec Pydantic validation."""
        # Valid spec with config as first-class field
        spec = ComponentInstanceSpec(path="interloper.destination.file.FileDestination", config={"base_path": "/tmp"})
        assert spec.path == "interloper.destination.file.FileDestination"
    ...
    def test_destspec_json_serialization(self):
...
class TestDestinationDefinitionSpec:
    """Test ComponentDefinitionSpec creation and serialization for Destination."""
