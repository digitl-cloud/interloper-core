...
class TestDAGSpec:
    """Test DAGInstanceSpec serialization and reconstruction."""
    ...
    def test_dagspec_roundtrip(self):
        ...
        # Convert to spec
        spec = dag.to_spec()
        assert isinstance(spec, DAGInstanceSpec)
        assert len(spec.assets) == 2
        assert all(isinstance(asset_spec, AssetInstanceSpec) for asset_spec in spec.assets)

        # Test JSON serialization
        json_str = spec.model_dump_json()
        parsed_spec = DAGInstanceSpec.model_validate_json(json_str)
        assert len(parsed_spec.assets) == 2
        assert all(isinstance(asset_spec, AssetInstanceSpec) for asset_spec in parsed_spec.assets)

    def test_dagspec_validation(self):
        """Test DAGInstanceSpec Pydantic validation."""
        # Valid spec
        asset_spec = AssetInstanceSpec(path="myapp.assets.my_asset")
        spec = DAGInstanceSpec(assets=[asset_spec])
        assert len(spec.assets) == 1

        # Invalid asset_specs
        with pytest.raises(ValidationError):
            DAGInstanceSpec(assets="not_a_list")  # type: ignore[arg-type]

        # Missing required field
        with pytest.raises(ValidationError):
            DAGInstanceSpec()  # type: ignore[call-arg]

    def test_dagspec_json_serialization(self):
        """Test DAGInstanceSpec JSON serialization."""
        asset_spec = AssetInstanceSpec(path="myapp.assets.my_asset")
        spec = DAGInstanceSpec(assets=[asset_spec])
        ...
        # Parse from JSON
        parsed = DAGInstanceSpec.model_validate_json(json_str)
    ...
    def test_spec_persistence(self):
        """DAGInstanceSpec can be saved and loaded from a JSON file."""
        asset_spec = AssetInstanceSpec(path="myapp.assets.my_asset")
        dag_spec = DAGInstanceSpec(assets=[asset_spec])
        ...
        try:
            ...
            loaded_spec = DAGInstanceSpec.model_validate(loaded_data)
