...
from interloper_google_cloud.destination.bigquery import BigQueryDestination, _bq_to_py_type, _py_to_bq_type
...
# ------------------------------------------------------------------
# BigQueryDestination.__init__
# ------------------------------------------------------------------


class TestBigQueryDestinationInit:
    """BigQueryDestination constructor and attribute storage."""

    @patch("interloper_google_cloud.destination.bigquery.service_account")
    @patch("interloper_google_cloud.destination.bigquery.bigquery.Client")
    def test_default_params(self, mock_client_cls, mock_sa):
        ...
        io = BigQueryDestination(project="my-project", service_account_key=_SA_KEY)
    ...
    def test_service_account_key_parsed(self, mock_client_cls, mock_sa):
        mock_creds = MagicMock()
        mock_sa.Credentials.from_service_account_info.return_value = mock_creds
        BigQueryDestination(project="p", service_account_key=_SA_KEY)
...
class TestResolveDataset:
    ...
    def test_schema_takes_precedence(self, _mock, _mock_sa):
        io = BigQueryDestination(project="p", default_dataset="fallback", service_account_key=_SA_KEY)
    ...
    def test_raises_when_both_none(self, _mock, _mock_sa):
        io = BigQueryDestination(project="p", service_account_key=_SA_KEY)
...
class TestTableRef:
    ...
    def test_with_explicit_schema(self, _mock, _mock_sa):
        io = BigQueryDestination(project="my-project", default_dataset="default_ds", service_account_key=_SA_KEY)
...
class TestToSpec:
    """Serialization of BigQueryDestination to ComponentInstanceSpec."""

    @patch("interloper_google_cloud.destination.bigquery.service_account")
    @patch("interloper_google_cloud.destination.bigquery.bigquery.Client")
    def test_minimal_spec(self, _mock, _mock_sa):
        io = BigQueryDestination(project="proj", service_account_key=_SA_KEY)
        spec = io.to_spec()

        assert isinstance(spec, ComponentInstanceSpec)
        assert spec.path == "interloper_google_cloud.destination.bigquery.BigQueryDestination"
    ...
    def test_spec_roundtrip(self, mock_client_cls, _mock_sa):
        """to_spec() output can reconstruct an equivalent BigQueryDestination."""
        original = BigQueryDestination(
        ...
        assert isinstance(reconstructed, BigQueryDestination)
...
class TestTableExists:
    ...
    def test_returns_true_when_found(self, mock_client_cls, _mock_sa):
        ...
        io = BigQueryDestination(project="p", default_dataset="ds", service_account_key=_SA_KEY)
